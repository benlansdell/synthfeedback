...
class NPModel4_CorrectBatch(BaseModel):
    #Four layers version
    def __init__(self, config):
        super(NPModel4_CorrectBatch, self).__init__(config)
        self.build_model()
        self.init_saver()
...
class NPModel4_ExactLsq_Linear_CorrectBatch(BaseModel):
    #Four layers version
    def __init__(self, config):
        super(NPModel4_ExactLsq_Linear_CorrectBatch, self).__init__(config)
...
class AENPModel5_ExactLsq(BaseModel):
    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            np_est1 = tf.matmul(tf.transpose(xi1),tf.diag(loss_p - loss)/var_xi/var_xi)
            np_est2 = tf.matmul(tf.transpose(xi2),tf.diag(loss_p - loss)/var_xi/var_xi)
            np_est3 = tf.matmul(tf.transpose(xi3),tf.diag(loss_p - loss)/var_xi/var_xi)
