...
var_xi=tf.placeholder(tf.float32,[1,1],name="Noise")
...
num=6
noise=np.logspace(-4,-1,num)
init = tf.global_variables_initializer()
iteration= 100000
store_al=np.zeros((num,iteration))
store_df=np.zeros((num,iteration))
store_err=np.zeros((num,iteration))
store_acc=np.zeros((num,iteration))
...
with tf.Session(config=config) as sess:
    ...
    sess.run(init)
    for no in range(np.shape(noise)[0]):
        pr_no=np.reshape(noise[no],[1,1])
        print("var_xi:",noise[no])
        for idx in range(iteration):
            (train_x, train_y) = mnist.train.next_batch(batch_size) 
            _,align,diff,err,acc=sess.run([train_step,alignment,norm_diff,loss_0,accuracy],feed_dict={x: train_x, y: train_y,var_xi:pr_no})
            
            if np.isnan(err)==True:
                print("\n\tModel does not converge!!!\n")
                break
#             store_out[idx,:] = out[0][:,0]
            store_err[no,idx]=err
            store_al[no,idx]=align
            store_acc[no,idx]=acc
        print("Completed")
     
with open("/home/prashanth/synthfeedback/Pickles/MNIST_NP_noise.pkl",'wb') as f:
    pickle.dump([store_err,store_al,store_acc,iteration,noise],f)